...
export default function Inventory() {
    ...
    // Form state (strings for inputs, to avoid uncontrolled issues)
    const [name, setName] = useState<string>("");
    const [description, setDescription] = useState<string>("");
    const [quantity, setQuantity] = useState<string>(""); // empty by default
    const [price, setPrice] = useState<string>("");       // empty by default
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        startTransition(async () => {
            ...
            // Check for the 'success' property in the result
            if (result?.success) {
                resetForm();
                const allItems = await getAllItems();
                if (allItems) setItems(allItems as unknown as ItemType[]);
    ...
    return (
        <div className="p-6">
            ...
            {/* Items Table */}
            <div>
                <h2 className="text-xl font-bold mb-4">Existing Items</h2>
